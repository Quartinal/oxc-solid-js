...
impl EventHandlers {
    ...
    pub fn check<'a>(&self, opening: &JSXOpeningElement<'a>) -> Vec<Diagnostic> {
        ...
        for attr in &opening.attributes {
            ...
            if let Some(correct) = self.get_correct_event_name(&lowercase) {
                if correct != name {
                    diagnostics.push(
                        ...
                        .with_fix(
                            Fix::new(name_span, correct.to_string()).with_message("Fix casing"),
                        ),
                    );
                    continue;
                }
            ...
            {
                let handler_name = format!("on{}{}", third_char.to_ascii_uppercase(), &name[3..]);
...
mod tests {
    ...
    fn test_get_correct_event_name() {
        ...
        assert_eq!(
            rule.get_correct_event_name("onmousedown"),
            Some("onMouseDown")
        );
