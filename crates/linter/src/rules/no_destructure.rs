...
use oxc_ast::ast::{ArrowFunctionExpression, Expression, Function, FunctionBody, Statement};
    ...
    fn statement_has_jsx(stmt: &Statement) -> bool {
        match stmt {
            Statement::ExpressionStatement(expr_stmt) => {
                Self::expression_has_jsx(&expr_stmt.expression)
            }
            Statement::ReturnStatement(ret) => ret
                .argument
                .as_ref()
                .is_some_and(|e| Self::expression_has_jsx(e)),
            ...
            Statement::IfStatement(if_stmt) => {
                Self::statement_has_jsx(&if_stmt.consequent)
                    || if_stmt
                        .alternate
                        .as_ref()
                        .is_some_and(|s| Self::statement_has_jsx(s))
            }
            _ => false,
        }
    }
    ...
    fn expression_has_jsx(expr: &Expression) -> bool {
        match expr {
            ...
            Expression::ParenthesizedExpression(paren) => {
                Self::expression_has_jsx(&paren.expression)
            }
