...
use oxc_ast::ast::{
    Expression, JSXAttributeName, JSXAttributeValue, JSXElement, ObjectPropertyKind, PropertyKey,
};
        ...
        for attr in &opening.attributes {
            if let oxc_ast::ast::JSXAttributeItem::Attribute(jsx_attr) = attr {
                ...
                if prop_name == "dangerouslySetInnerHTML" {
                    diagnostics.push(
                        self.handle_dangerously_set_inner_html(jsx_attr.span, &jsx_attr.value),
                    );
                    continue;
                }
        ...
        if let Some(JSXAttributeValue::ExpressionContainer(container)) = value {
            if let Some(Expression::ObjectExpression(obj)) = container.expression.as_expression() {
                if obj.properties.len() == 1 {
                    if let Some(ObjectPropertyKind::ObjectProperty(prop)) = obj.properties.first() {
