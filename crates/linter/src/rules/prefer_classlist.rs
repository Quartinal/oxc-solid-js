...
use oxc_ast::ast::{Argument, Expression, JSXAttributeName, JSXAttributeValue, JSXOpeningElement};
use oxc_span::Span;
...
impl PreferClasslist {
    ...
    fn get_classnames_call_info<'a>(&self, expr: &'a Expression<'a>) -> Option<(&'a str, Span)> {
        let call = match expr {
