...
use oxc_ast::ast::{
    ConditionalExpression, Expression, JSXChild, JSXElement, JSXExpressionContainer, JSXFragment,
    LogicalExpression,
};
    ...
    pub fn check_expression_container<'a>(
        ...
        match expr {
            Expression::LogicalExpression(logical) => {
                diagnostics.extend(self.check_logical_expression(logical, container.span, source));
            }
                        ...
                        arrow.body.statements.first()
                    {
                        match &expr_stmt.expression {
                            ...
                            Expression::ConditionalExpression(cond) => {
                                diagnostics.extend(
                                    self.check_conditional_expression(cond, cond.span, source),
                                );
                            }
                            _ => {}
                        }
            ...
            Expression::ConditionalExpression(cond) => {
                diagnostics.extend(self.check_conditional_expression(cond, container.span, source));
            }
            _ => {}
        }
