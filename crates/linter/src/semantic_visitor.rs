...
use oxc_ast::ast::{
    Argument, ArrowFunctionExpression, CallExpression, Expression, Function, ImportDeclaration,
    ImportDeclarationSpecifier, JSXElementName, JSXMemberExpressionObject, JSXOpeningElement,
    Program, Statement,
};
...
impl<'a> SemanticLintRunner<'a> {
    ...
    fn current_scope(&self) -> ScopeId {
        *self
            .scope_stack
            .last()
            .unwrap_or(&self.semantic.scoping().root_scope_id())
    }
            ...
            if let oxc_ast::ast::JSXAttributeItem::Attribute(jsx_attr) = attr {
                if let oxc_ast::ast::JSXAttributeName::NamespacedName(ns) = &jsx_attr.name {
                    if ns.namespace.name == "use" {
                        ...
                        self.check_jsx_identifier(directive_name, ns.name.span, scope_id, false);
                    }
                }
            }
        }
    ...
    fn check_function_component(&mut self, func: &Function<'a>) {
        ...
        let returns_jsx = func
            .body
            .as_ref()
            .is_some_and(|body| NoDestructure::body_has_jsx(body));
        ...
        if is_pascal_case || returns_jsx || is_known_component {
            ...
            if func.body.is_some() {
                self.diagnostics
                    .extend(rule.check_function(func, true, self.is_inside_jsx()));
            }
        }
    }
    ...
    fn check_arrow_component(&mut self, arrow: &ArrowFunctionExpression<'a>) {
        ...
        if self.config.components_return_once {
            let rule = ComponentsReturnOnce::new();
            self.diagnostics
                .extend(rule.check_arrow(arrow, true, self.is_inside_jsx()));
        }

        if self.config.no_destructure {
            let rule = NoDestructure::new();
            self.diagnostics
                .extend(rule.check_arrow(arrow, returns_jsx, self.is_inside_jsx()));
        }
    }
    ...
    fn visit_function(&mut self, func: &Function<'a>, _flags: oxc_syntax::scope::ScopeFlags) {
        ...
        if self.config.no_destructure && !self.is_inside_jsx() {
            let returns_jsx = func
                .body
                .as_ref()
                .is_some_and(|b| NoDestructure::body_has_jsx(b));
            if returns_jsx {
                let rule = NoDestructure::new();
                self.diagnostics.extend(rule.check_function(
                    func,
                    returns_jsx,
                    self.is_inside_jsx(),
                ));
...
mod tests {
    ...
    fn test_auto_import_suggestion() {
        ...
        assert!(result
            .diagnostics
            .iter()
            .any(|d| d.message.contains("solid-js")));
    }
    ...
    fn test_custom_directive_defined() {
        ...
        assert!(!result
            .diagnostics
            .iter()
            .any(|d| d.message.contains("myDirective") && d.message.contains("not defined")));
    }
    ...
    fn test_reactivity_non_function_arg() {
        ...
        assert!(result
            .diagnostics
            .iter()
            .any(|d| d.message.contains("createEffect") && d.message.contains("function")));
    }
