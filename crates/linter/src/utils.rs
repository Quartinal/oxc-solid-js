...
use oxc_ast::ast::{
    JSXAttribute, JSXAttributeItem, JSXAttributeName, JSXChild, JSXElementName,
        ...
        if let JSXAttributeItem::Attribute(jsx_attr) = attr {
            match &jsx_attr.name {
                ...
                JSXAttributeName::NamespacedName(ns) => {
                    result.push((format!("{}:{}", ns.namespace.name, ns.name.name), ns.span));
                }
            }
        }
    }
...
pub const REACT_PROP_REPLACEMENTS: &[(&str, &str)] = &[("className", "class"), ("htmlFor", "for")];
