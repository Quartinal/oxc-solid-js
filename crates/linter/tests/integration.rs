...
fn test_self_closing_comp_with_children() {
    ...
    assert!(
        ...
        "should have no diagnostics - has children"
    );
}
...
fn test_no_innerhtml_dangerously_set() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "should warn about dangerouslySetInnerHTML"
    );
...
fn test_no_unknown_namespaces_on_component() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "namespaces on components have no effect"
    );
...
fn test_prefer_classlist_already_has_classlist() {
    ...
    assert!(
        diagnostics.is_empty(),
        "should not warn when classList exists"
    );
}
...
fn test_prefer_classlist_plain_class() {
    ...
    assert!(
        diagnostics.is_empty(),
        "should not warn about plain class strings"
    );
}
...
fn test_no_react_deps_valid_with_initial_value() {
    ...
    assert!(
        diagnostics.is_empty(),
        "function with params and initial value should be valid"
    );
}
...
fn test_no_react_deps_valid_memo_single_arg() {
    ...
    assert!(
        diagnostics.is_empty(),
        "single argument memo should be valid"
    );
}
...
fn test_no_react_deps_valid_other_function() {
    ...
    assert!(
        diagnostics.is_empty(),
        "should not warn about other functions"
    );
}
